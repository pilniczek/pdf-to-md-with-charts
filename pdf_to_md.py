...
def process(pdf_path: str, output_md: str) -> None:
    ...
    try:
        print(f"[1/3] Converting PDF to Markdown (extracting images to {img_dir}) …")
        ...
        # pymupdf4llm writes:  ![](path/to/image.png)
        ...
        for i, m in enumerate(matches):
            ...
            # The path in the markdown may be relative; resolve it
            img_abs_path = img_ref_path
            if not os.path.isabs(img_ref_path):
                # Try relative to cwd, then relative to img_dir
